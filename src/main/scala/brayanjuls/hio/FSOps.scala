...
object mkdir{
  def apply(path: HIOPath, permission:FsPermission = FileContext.DIR_DEFAULT_PERM,
 ...
 */
object write {
  def apply(path:HIOPath, fileContent:FileContent,
...
object ls{
  ...
  def apply(path:HIOPath, filterFunction:Path => Boolean = _ => true,
    ...
    val fileStatusIterator = getFileStatus(path, filterFunction, configuration)
      ...
      case e: FileNotFoundException => throw HIOValidationError(s"The Path $path does not exists",e)
    }
  }

  private def getFileStatus(path: HIOPath, filterFunction: Path => Boolean = _ => true,
  ...
  private def getFileStatusWithWildCard(path: HIOPath, configuration: Configuration = getFSConfiguration) = {
    FileContext
      .getFileContext(path.uri, configuration)
      .util()
  ...
  def withWildCard(path: HIOPath, configuration:Configuration = getFSConfiguration): Seq[String] = {
    try {
  ...
  object status{
    ...
    def withWildCard(path: HIOPath, configuration: Configuration = getFSConfiguration): Array[FileStatus] = {
      try {
...
object remove{
  def apply(path: HIOPath, configuration:Configuration = getFSConfiguration):Boolean = {
    FileContext
      .getFileContext(path.uri,configuration)
...
object read{
  def apply(path:HIOPath, configuration:Configuration = getFSConfiguration): FSDataInputStream = {
    FileContext
      .getFileContext(path.uri,configuration)
  ...
  object string{
    def apply(path: HIOPath, configuration:Configuration = getFSConfiguration): String = {
      val outputStream = FileContext
        .getFileContext(path.uri,configuration)
        .open(path.relativePath)
      val fileContent = IOUtils.toString(outputStream, FileContent.CHARSET)
      outputStream.close()
      fileContent
    }
  }
...
object copy{
  ...
  def apply(originPath:HIOPath, destinationPath:HIOPath, overWrite:Boolean = false,
    ...
    (originPath.ext.isEmpty, destinationPath.ext.isEmpty) match {
      ...
      case (true, false) =>
        throw HIOValidationError(s"can't ${if(deleteSource) "move" else "copy"} all the files of directory ${originPath.toString} into a single file ${destinationPath.toString}.")
    }
  }

  def withWildCard(originPath:HIOPath, destinationPath:HIOPath, overWrite: Boolean = false,
                   configuration: Configuration = getFSConfiguration, deleteSource:Boolean = false): Unit = {
    ...
    (paths.isEmpty, destinationPath.ext.isEmpty) match {
      ...
      case (false,false) =>
        paths.length match {
          ...
          case _ => throw HIOValidationError(s"can't copy all the files of ${originPath.toString} into a single file ${destinationPath.toString}.")
        }
  ...
  def apply(originPath:HIOPath, destinationPath:HIOPath, overWrite:Boolean = false,
    ...
    if (originPath.baseName.toString == destinationPath.baseName.toString){
      (originPath.ext.isEmpty, destinationPath.ext.isEmpty) match {
        ...
        case (true, false) =>
          throw HIOValidationError(s"can't move all the files of directory ${originPath.toString} into a single file ${destinationPath.toString}.")
      }
    }else{
      ...
      (paths.isEmpty, destinationPath.ext.isEmpty) match {
        ...
        case (false, false) =>
          paths.length match {
            ...
            case _ => throw HIOValidationError(s"can't move all the files of ${originPath.toString} into a single file ${destinationPath.toString}.")
          }
        case (true, _) =>
      }
    }else{
